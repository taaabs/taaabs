  ...
  if (request.action == 'send-chatbot-prompt') {
      ...
      try {
        ...
        if (params.resolve) {
          try {
            ...
            } else if (current_url == chatbot_urls.huggingchat) {
              if (
                document.querySelector(
        ...
        setTimeout(() => send_prompt(params), 500)
      }
    }
    ...
    if (
      ...
      (current_url == chatbot_urls.chatgpt ||
        current_url == chatbot_urls.duckduckgo ||
      ...
      const send_prompt_sequentially = async () => {
        for (let i = 0; i <= prompt_parts.length; i++) {
          ...
          await new Promise((resolve) =>
            send_prompt({ prompt, is_first_part: i == 0, resolve }),
          )
        }
      }
