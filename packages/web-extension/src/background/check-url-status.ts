...
async function derive_hash_from_url(url: string, encryption_key: Uint8Array): Promise<string> {
  return SHA256(url, encryption_key);
}

export const SHA256 = async (data: string, encryption_key: Uint8Array): Promise<string> => {
  const encoder = new TextEncoder();
  const encoded_data = encoder.encode(data);
  const merged_array = new Uint8Array(encoded_data.length + encryption_key.length);
